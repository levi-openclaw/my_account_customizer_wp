...
class WCMembershipCompatibility {
	...
	public function maybe_render_unified_view( $value ) {
		...
		// --- Tab switching JS ---
		?>
		<script id="tgwc-membership-tabs-v2.3.0">
		(function() {
			...
			function activateTab(tabId) {
				...
				panels.forEach(function(p) {
					p.style.display = (p.id === 'tgwc-tab-' + tabId) ? '' : 'none';
				});
				return found;
			}

			// Tab click — set hash + switch tab.
			...
			// On load — activate tab from URL hash if present.
			var hash = window.location.hash.replace('#', '');
			if (hash && !activateTab(hash)) {
				// Hash didn't match any tab — keep default (first tab active).
			}
		})();
		</script>
		<?php
	...
	private function render_combined_discounts( $memberships ) {
		...
		?>
		<style id="tgwc-discount-cards-v2.3.0">
			/* tgwc-discount-cards v2.3.0 */
		...
		?>
		<style id="tgwc-cart-toast-v2.3.0">
		...
		<script id="tgwc-ajax-cart-v2.3.0">
	...
	public function output_membership_css() {
		...
		?>
		<style id="tgwc-memberships-compat-v2.3.0">
			/* tgwc-memberships-compat v2.3.0 */
			/* ================================================================
			   Gamut Design Tokens
			   ================================================================ */
